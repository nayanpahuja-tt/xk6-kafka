...
4. Build the binary with CGO enabled:
   ...
   ```shell
   CGO_ENABLED=1 xk6 build v2.2.0 --with  github.com/nayanpahuja-tt/xk6-kafka/v2@latest
   ```

> [!NOTE]
> Go modules require a `/v2` import path for major version 2 and later. Use ` github.com/nayanpahuja-tt/xk6-kafka/v2@…` for `v2.x.x` tags and `github.com/mostafa/xk6-kafka@…` only for `v1.x.x` and earlier.
...
2. Build the custom binary. On Mac, make sure to add the `GOOS=darwin` option.
   ...
   docker run --rm -e GOOS=darwin -u "$(id -u):$(id -g)" -v "${PWD}:/xk6" \
       ...
       --with  github.com/nayanpahuja-tt/xk6-kafka/v2@latest
   ```
